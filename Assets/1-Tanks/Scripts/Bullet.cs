...
namespace Tanks
{
    public class Bullet : MonoBehaviour // This is checking for Health script in that if there is damage, it will reduce the health
    {
        ...
        private Rigidbody2D rBody; // Reference to Rigidbody2D
        ...
        void Explode()
        {
            ...
            foreach (var hit in hits)
            {
                ...
                if (health)
                {
                    // Create a temp damage variable
                    // Calculate distance between ourselves and hit thing
                    // Subtract tempDamage with distance
                    // Deal damage to that thing!
            ...
            if (tilemap)
            {
                for (int x = -radius; x <= radius; x++)
                {
                    for (int y = -radius; y <= radius; y++)
                    {
                        ...
                        float distance = Vector3.Distance(point, hitPoint);
